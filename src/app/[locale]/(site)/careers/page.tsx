      ...
      <SectionContainer className="bg-white dark:bg-slate-900">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
          <h2 className="text-2xl font-black text-slate-900 dark:text-white">
            Current Openings <span className="ml-2 text-primary text-sm font-bold bg-primary/5 dark:bg-primary/10 px-3 py-1 rounded-full">{jobs.length}</span>
          </h2>
          <div className="flex gap-3">
            <select className="px-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-xl text-sm font-bold text-slate-500 dark:text-slate-400 focus:outline-none">
        ...
        <div className="space-y-4">
          {jobs.length > 0 ? jobs.map((job) => (
            <Link
              ...
              className="group flex flex-col md:flex-row md:items-center justify-between p-8 bg-white dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700 rounded-[32px] transition-all hover:bg-slate-50 dark:hover:bg-slate-800 hover:border-primary/20 hover:shadow-card"
            >
              ...
              <div className="flex items-center gap-4">
                <div className="hidden sm:flex flex-col items-end mr-6 text-right">
                  <span className="text-xs font-bold text-slate-300 dark:text-slate-600 uppercase tracking-widest mb-1">Apply by</span>
                  <span className="text-sm font-bold text-slate-500 dark:text-slate-400">{job.closing_date ? new Date(job.closing_date).toLocaleDateString(locale) : 'TBD'}</span>
                </div>
                <div className="size-14 rounded-2xl bg-slate-900 dark:bg-slate-700 text-white flex items-center justify-center transition-all group-hover:bg-primary group-hover:shadow-glow-sm">
                  <LucideArrowRight size={24} />
                </div>
              </div>
            </Link>
          )) : (
            <div className="py-24 text-center border-2 border-dashed border-slate-100 dark:border-slate-800 rounded-[40px]">
              <LucideBriefcase size={48} className="text-slate-200 dark:text-slate-700 mx-auto mb-4" />
              <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">No active openings right now</h3>
