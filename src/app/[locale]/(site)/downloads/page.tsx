           ...
           <div className="lg:col-span-1 space-y-10">
              <div className="relative">
                 ...
                 <input 
                  ...
                  className="w-full bg-white border border-slate-200 rounded-2xl pl-12 pr-4 py-3 text-sm focus:ring-4 focus:ring-primary/10 transition-all outline-none"
                 />
              </div>

              <div className="space-y-4">
                 <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest ml-4">Categories</h4>
                 <div className="flex flex-col gap-1">
                    ...
                    {categories.length > 0 ? categories.map((cat) => (
                      <button key={cat.id} className="flex items-center justify-between px-4 py-3 text-slate-600 hover:bg-slate-100 rounded-2xl font-bold text-sm transition-colors text-left">
                        {cat.name}
                      ...
                      <div className="space-y-2 mt-4">
                        {["Annual Reports", "CSR Policies", "Media Kit", "Tender Forms"].map((c, i) => (
                          <button key={i} className="flex items-center justify-between px-4 py-3 text-slate-600 hover:bg-slate-100 rounded-2xl font-bold text-sm transition-colors text-left w-full">
                            {c}
                            <LucideChevronRight size={16} className="text-slate-200" />
                          </button>
                        ))}
                      </div>
                    )}
           ...
           <div className="lg:col-span-3">
              <div className="grid sm:grid-cols-2 gap-6">
                 {resources.length > 0 ? resources.map((res) => (
                   <div key={res.id} className="group p-6 bg-white border border-slate-100 rounded-[32px] shadow-sm hover:shadow-xl hover:border-primary/20 transition-all">
                      <div className="flex items-start justify-between gap-4 mb-6">
                         ...
                         <a 
                          ...
                          className="size-10 rounded-xl bg-slate-50 text-slate-400 flex items-center justify-center hover:bg-slate-900 hover:text-white transition-all"
                         >
                    ...
                    [1,2,3,4,5,6].map((i) => (
                      <div key={i} className="group p-6 bg-slate-50 border border-slate-100 rounded-[32px] opacity-60">
                         <div className="size-14 bg-slate-200 rounded-2xl mb-6" />
