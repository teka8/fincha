      ...
      <SectionContainer className="bg-white dark:bg-slate-900">
        <div className="flex items-center gap-4 mb-12 flex-wrap">
          {["All", "Photos", "Videos", "Documents"].map((filter, i) => (
            <button
              key={i}
              className={`px-6 py-2.5 rounded-full text-sm font-bold transition-all ${i === 0 ? 'bg-primary text-white shadow-glow-sm' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700'}`}
            >
              {filter}
            </button>
          ))}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {mediaItems.length > 0 ? mediaItems.map((item, i) => (
            <div key={i} className="group relative aspect-square rounded-[32px] overflow-hidden bg-slate-100 dark:bg-slate-800 shadow-sm">
              {item.thumbnail ? (
                <Image
                  src={item.thumbnail}
                  alt={item.title}
                ...
                <div className="size-full bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-800 dark:to-slate-900 flex items-center justify-center text-slate-300 dark:text-slate-700">
            ...
            [1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
              <div key={i} className="group relative aspect-square rounded-[32px] overflow-hidden bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700">
                <div className="size-full flex items-center justify-center text-slate-200 dark:text-slate-700">
      ...
      <div className="bg-slate-50 dark:bg-slate-900/50">
        <SectionContainer className="py-20">
          <div className="max-w-layout mx-auto grid md:grid-cols-2 gap-12 items-center">
            <div>
              <h2 className="text-3xl font-black mb-6 dark:text-white">Annual Reports & <br /><span className="text-primary">Corporate Downloads</span></h2>
              <p className="text-muted dark:text-slate-400 leading-relaxed mb-8">Access our latest environmental impact assessments, annual production reports, and official branding guidelines.</p>
              <Link href={{ pathname: "/downloads" }} className="inline-flex items-center gap-3 font-bold text-slate-900 dark:text-slate-200 hover:text-primary transition-colors">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {[
                ...
                <div key={i} className="p-6 rounded-3xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm flex items-center gap-4 transition-transform hover:-translate-y-1">
                  ...
                  <div className="overflow-hidden">
                    ...
                    <p className="text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-widest">{doc.size} • PDF</p>
                  </div>
                </div>
              ))}
            </div>
