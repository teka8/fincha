...
export function HistoricalTimeline() {
  ...
  return (
    <div className="bg-white py-8 relative" ref={containerRef}>
      <SectionHeading 
        eyebrow={t("eyebrow")}
        title={t("title")}
      ...
      <div className="max-w-5xl mx-auto px-8 relative">
        {/* Vertical Line */}
        <div className="absolute left-[28px] md:left-1/2 inset-y-0 w-0.5 bg-slate-100 rounded-full z-0" />
        ...
        <div className="space-y-6 md:-space-y-4">
          {milestones.map((milestone, idx) => {
            ...
            return (
              ...
              >
                {/* Center Node */}
                <div className="absolute left-0 md:left-1/2 md:-translate-x-1/2 size-14 rounded-full bg-white border-4 border-slate-100 flex items-center justify-center z-10 shadow-sm">
                ...
                <div className={`w-full md:w-[45%] pl-16 md:pl-0 ${isEven ? 'md:pr-10 text-left md:text-right' : 'md:pl-10 text-left'}`}>
                  <div className="p-4 md:p-6 rounded-2xl md:rounded-[24px] bg-slate-50 border border-slate-100 hover:shadow-lg hover:border-primary/20 hover:bg-white transition-all duration-300 group">
                    <div className={`flex flex-col md:flex-row md:items-baseline gap-1 md:gap-3 mb-2 ${isEven ? 'md:justify-end' : ''}`}>
                      <span className="text-3xl font-black text-slate-200 group-hover:text-primary transition-colors tracking-tighter shrink-0">
                        {milestone.year}
                      </span>
